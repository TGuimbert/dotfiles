...
- New capability files for owner-less apps: `modules/kubernetes.nix` (`kubectl kubelogin fluxcd kind kubectx` + `.kube`), `modules/gui-apps.nix` (discord/signal/vlc/tidal-hifi/mullvad-browser/remmina/obsidian — the GUI counterpart to `cli-tools.nix`), `modules/cad.nix` (freecad/orca-slicer/sweethome3d/calibre), `modules/secrets-tools.nix` (yubikey-manager/rage/age-plugin-yubikey/sops/bitwarden-cli/bws), a dev-home aspect for uv/pip/npm config.
- `modules/home-base.nix` (collector on **`homeManager.modules.gui`**, matching every R7–R11 home module — `base` would leak desktop dirs onto srv-01) for the generic remainder: `Documents Downloads Music Pictures Videos Workspace .dotfiles .aws`. (`home.username`/`homeDirectory` are *not* re-declared — the home-manager NixOS module sets them from the user.)
- **`.minikube`/`.docker/config.json` go to *both* `podman.nix` and `docker.nix`** (each host-scoped inside its named aspect), since tuxedo runs minikube under docker — putting them only in podman.nix would regress tuxedo.
- **Steam persistence stays in a reduced `home/default.nix`** (only `.local/share/Steam .steam .clonehero ".config/unity3d/srylain Inc_/Clone Hero"`), still imported by leshen/griffin; it is migrated to `steam.nix` and the file deleted in **R13**. Drop `../../home` from **tuxedo** only (it keeps `../../home/work.nix`). Turn the `lib.mkDefault home.stateVersion` in `users.nix` into a plain assignment (tuxedo home 22.11 → 25.11 — intended).
- **Verify**: `nix flake check`; `nh os build .#leshen` then `nh os test`.
...
- [x] **R14**: Server services (`hosts/srv-01/{default,traefik,authelia,lldap,homepage,restic,calibre,printing}.nix` → `modules/server/base.nix` baseline collector on `nixos.modules.server` + per-service named aspects `nixos.modules.{traefik,authelia,lldap,homepage,restic,calibre,printing}`; srv-01 sops baseline landed; dropped dead `args.specialArgs`; deleted flat `modules/server.nix`; `hosts/srv-01/{hardware,disks}.nix` relocate in R15)
